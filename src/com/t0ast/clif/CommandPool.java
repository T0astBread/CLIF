 ...
 * can't be invoked from the command line. Methods that should appear as a command
 * should be annotated with {@link Command}. They may follow special naming conventions
 * where the first letter of a new word isn't capitalized but instead, every finished
 * word (except the last one) is followed by a _ (underscore). These underscores
 * are replaced with dashes (-) when the command is called on the command line.
 * </p>
 *
 * <p>
 * The commands in a single command pool should be related to each other and use
 * the same data/model objects. For exa
 * </p>
 *
 * @author T0astBread
 */
